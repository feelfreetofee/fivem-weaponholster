local weaponData = { -- Thanks to https://github.com/itzhapp/ShowWeapon
	...
	left = {
		list = {
			...
			['WEAPON_BOTTLE']				= {model = 'w_me_bottle', 			pos = vector3(0.0, 0.0, 0.0), 	rot = vector3(0.0, 0.0, 0.0)},
			...
			['WEAPON_HATHCET']				= {model = 'w_me_hatchet', 			pos = vector3(0.0, 0.0, 0.0), 	rot = vector3(0.0, 0.0, 0.0)},
			...
			['WEAPON_BATTLEAXE']			= {model = 'prop_tool_fireaxe', 	pos = vector3(0.0, 0.0, 0.0), 	rot = vector3(0.0, 0.0, 0.0)},
        ...
        if weaponEquipped and weaponEquipped.name then
			for k in pairs(weaponData) do
				if weaponData[k].list[weaponEquipped and weaponEquipped.name] then
					...
					AttachEntityToEntity(weaponHolstered[k].object, playerPed, GetPedBoneIndex(playerPed, 24816), pos, rot, true, true, false, false, 1, true)
